/**
 * St. Olaf Intramurals — standalone installer.
 * Replicates the base server's /api/appmgr/install flow:
 *   create kapp -> create forms (with pages) -> build & poll indexes -> seed.
      ...
      for (const formSlug of Object.keys(seedData)) {
        ...
        for (let i = 0; i < records.length; i += 10) {
          ...
          const results = await Promise.allSettled(batch.map(values =>
            kineticRequest("POST", `/kapps/${kappSlug}/forms/${formSlug}/submissions`, { values, coreState: "Submitted" })));
          for (const r of results) {
            if (r.status === "fulfilled" && r.value.status < 300) ok++;
            else { fail++; if (!firstErr) firstErr = r.status === "fulfilled" ? `${r.value.status} ${JSON.stringify(r.value.data).slice(0,160)}` : String(r.reason); }
          }
        }
        log(`✓ Seeded ${formSlug}: ${ok}/${records.length}${fail ? ` (${fail} failed — ${firstErr})` : ""}`);
      }
    } else log("• No seed-data.json found");
  }
